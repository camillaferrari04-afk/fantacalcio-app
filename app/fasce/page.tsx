...
export default function PaginaFasce() {
  ...
  // Gestione Acquisto veloce direttamente dalla vista Fasce
  ...
  // Rimuovi fascia o cambia fascia
  ...
  // Calciatori filtrati per il ruolo attivo
  const calciatoriRuolo = calciatori.filter(
    (c) => c.r?.toUpperCase() === ruoloSelezionato
  );

  // Mappa delle fasce disponibili (1 to 5)
  const fasceUniche = [1, 2, 3, 4, 5];
        ...
        {calciatoriRuolo.length === 0 && (
          <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-slate-300">
            <p className="text-slate-400 font-medium text-sm">
              Nessun giocatore inserito in fascia per questo ruolo.
            </p>
            <p className="text-xs text-slate-400 mt-1">
              Assegna le fasce (F1, F2...) dalla Home Listone per vederli qui!
            </p>
          </div>
        )}
      </div>
